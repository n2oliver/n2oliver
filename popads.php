...
class PopAdsAdcode {/* Monetização equilibrada */
	var $minBid = 0.00015;          // Evita anunciantes lixo, melhora eCPM
	var $popundersPerIP = 3;        // 1 pop por IP (não irrita)
	var $delayBetween = 180;        // 3 minutos entre tentativas
	var $defaultPerDay = 1;         // 1 pop por usuário por dia
	var $topmostLayer = 'auto';     // Deixa o PopAds decidir melhor
	var $blockedCountries = null;   // Bloqueie países direto no painel, não aqui

	/* URL padrão */
	var $default = false;

	/* Seus dados */
	var $key = '4d3a91b666f691014aaf3dd1a7a854e801b3b935';
	var $siteId = 5215724;

	/* Anti bloqueio */
	var $antiAdblock = 1;           // BOM manter
	var $obfuscate = 1;             // BOM manter

	/* SSL */
	var $ssl = true;                // ⚠️ ATIVE se seu site usa https

	/* Debug */
	var $verbose = false;           // ❌ desligar em produção
	var $adcodeDir = false;

	/* Conexões (OK manter padrão) */
	var $curlTimeout = 5;
